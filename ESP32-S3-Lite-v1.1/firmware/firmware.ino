/*
 * ============================================================================
 * ESP32 S3 Glasses Scanner Lite v1.1
 * ============================================================================
 * Consumer smart-glasses scanner derived from the hardware-validated CYD v6.3
 * lineage. The CYD remains the development/database-building platform; this
 * Lite v1.1 keeps the consumer scanner behavior while adding a bounded,
 * privacy-focused internal-flash field-test log for >=40% classifications.
 *
 * NO SD / CSV / manual capture / Sentry / TFT / self test
 * NO suspicious-device development engine
 * known_devices.h is retained as annotation-only consumer context
 *
 * Retained from CYD v6.3:
 *   - active BLE scan ~2 s every ~5 s
 *   - BLE interval/window 100/95 ms
 *   - v6.3 HIGH/MEDIUM/LOW smart-glasses rules and false-positive suppression
 *   - exact Anko43700141 = 89 POSSIBLE rule
 *   - HIGH alert boundary remains 97
 *   - RSSI is not used as a detection gate
 *   - PRIVATE NRPA via Arduino-ESP32 3.3.11 NimBLE, random 12-18 minute rotation between scans
 *
 * Lite-specific consumer behaviour:
 *   - onboard RGB LED is the primary user interface
 *   - 3 failed PRIVATE-NRPA startups -> PUBLIC scan fallback
 *   - PUBLIC fallback is solid purple for the entire fallback session
 *   - camera/audio review engine uses explicit BLE-name clues at 90% review
 *   - non-red LED colours are dim; urgent red alerts use a full-brightness strobe
 *   - >=40% candidates are deduplicated per session and persisted to LittleFS
 *   - raw observed BLE MAC addresses are never persisted
 *   - LOG is the single serial retrieval command for field-test history
 *
 * IMPORTANT: camera/audio 90% is rule-match confidence. It does not establish
 * that a device is hidden, recording, malicious, or being used for surveillance.
 * ============================================================================
 */
