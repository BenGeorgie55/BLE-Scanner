...
## Build-specific scope — ESP32-S3 Lite v1.1

The S3 Lite v1.1 is a portable BLE detector with internal LittleFS field-test logging.

Current-source behaviour relevant to privacy/legal use:
- observations at or above the configured logging threshold can be retained;
- records are deduplicated within a session;
- raw observed BLE MAC addresses are not intentionally written to LittleFS or NVS;
- a fresh RAM-only salt is used for session-scoped pseudonymous correlation;
- session/reset/privacy diagnostic history can survive reboot or power loss;
- `LOG` can retrieve retained history;
- normal detection does not require a companion app, Wi-Fi or cloud processing.

Because records survive reboot, exported `LOG` output should be treated as potentially sensitive operational data even though observed-device correlation is session scoped.

The S3 normally uses a rotating private scanner address, but after repeated private-address setup failures it can enter a visibly indicated public-address fallback. Public fallback is a **degraded privacy state**, not equivalent to private scanning.

Recommended public positioning: portable consumer/privacy-awareness detector and field-test tool. Organisational use should follow the workplace, school, healthcare, aged-care and secure-facility cautions above.
