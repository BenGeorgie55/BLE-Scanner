...
## Build-specific scope — CYD Expanded DEV 1 of 2 (CYD)

**1 OF 2 MUST BE FLASHED TO THE ESP32-2432S028R CYD.**

This is the scanning/controller half of the experimental two-board architecture. It remains the authoritative local BLE scanner and detection engine.

### Experimental status

This build is working/development firmware but remains mostly untested compared with CYD DEV Lite. It must not be represented as a certified security, safety, law-enforcement or compliance appliance.

### UART lookup data

The CYD can send supplementary database lookup requests to the separate database ESP32 over local 3.3 V UART.

A raw observed BLE MAC address may exist transiently in RAM and may transit the UART request where required for exact matching, OUI extraction or deduplication. It must not be intentionally persisted by either half.

### External 8 dBi antenna

This build is intended to use an external higher-gain antenna. Antenna gain and installation can alter effective radiated power and radio characteristics.

Do not state that an 8 dBi antenna is automatically compliant on every ESP32/CYD module. The complete radio/module/antenna configuration should be checked against applicable ACMA equipment and LIPD requirements.

### Busy-mode RSSI priority

Busy mode can prioritise database service from stronger RSSI toward weaker RSSI. This is queue servicing, **not a reliable distance measurement**. Do not use priority order as evidence that a person/device is physically closest.

### Database failure

The external database is supplementary. Local CYD rules remain authoritative and continue if the coprocessor fails. A database failure reduces supplementary lookup capability; it does not create a legal conclusion about any observation.

Recommended public positioning: authorised stationary facility/development/research build, with explicit organisational approval for schools, childcare, healthcare, aged care, workplaces, correctional or secure facilities.
