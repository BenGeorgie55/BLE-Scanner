/*
 * CYD V8 — Smart Glasses Detector + BLE Database Client + Private-Rotating BLE Scanner
 * V8 — CYD half (1/2) of the BLE database-coprocessor build. Hardware validation required after flash.
 * Confirmed-working hardware baseline: v5; v6.1 core has also been hardware-tested.
 * Target: ESP32-2432S028R ("Cheap Yellow Display" / CYD)
 *
 * Based on / inspired by:
 *   surveillancewatch/ESP-GlassHole: https://github.com/surveillancewatch/ESP-GlassHole/tree/main/firmware
 *   sh4d0wm45k/glass-detect: https://github.com/sh4d0wm45k/glass-detect/
 *
 * v6.1 preserves the v3.4 scanner/SD baseline while updating self-test to current v6.1 UI/LED semantics
 * except where explicitly superseded by the confidence engine and v6.1 UI rules.
 * Legacy glasses_config.h / generic_keyword_matches.h are not compiled in v6.2.
 *

 * v6.1 is derived from the confirmed-working v5 hardware baseline. Sentry Mode
 * behaviour remains inherited from that baseline:
 *   Configured BLE sleep -> activity sample -> fixed ACTIVE learning period
 *   when activity is significantly above baseline. Ordinary wake samples never
 *   drift an established baseline; the completed ACTIVE-hour median replaces it.
 *

 * v6.1 manual context logging:
 *   Holding BOOT on the Scanner page freezes the 5 immediately preceding BLE
 *   observations and then captures the next 5 in arrival order. The manual CSV
 *   omits RSSI/range fields; live detection still processes the full advertisement.
 *
 * Detection / alert policy:
 *   confidence 1..59  = LOW manufacturer clue; log/annotate only, no alert
 *   confidence 60..96 = POSSIBLE; orange LED + orange TFT banner
 *   confidence 97..100 = HIGH; solid red LED + red tiled TFT alert
 * No RSSI threshold suppresses a confidence match. RSSI is informational only.
 *
 * SD files (card root):
 *   /advertisements.csv
 *   /detections.csv
 *   /manual_smart_glasses_logged.csv
 *   /sessions.csv
 */
...
// CYD V8 display validation: this build MUST use ILI9341.
...
// ============================================================
// V8 — Local BLE Scanner Address Privacy (inherited from validated v6.3 RF baseline)
// ============================================================
...
void drawLegalPrivacyNotice(bool includeCredits,
    ...
    // User action instructions are intentionally dark blue.
    tft.setTextSize(1);
...
void drawBootCreditsPage() {
    ...
    tft.setTextSize(2);
    tft.setCursor(58, 5);
    tft.print("BLE/WiFi SNIFFER");
    tft.setCursor(47, 25);
    tft.print("BUILT BY BEN ELTIS");

    tft.setTextSize(1);
    tft.setTextColor(TFT_CYAN, TFT_BLACK);
    tft.setCursor(8, 52);
    tft.print("CREDITS / INSPIRATION:");

    tft.setTextColor(TFT_WHITE, TFT_BLACK);
    const char* creditLines[] = {
        "surveillancewatch/ESP-GlassHole",
        "sh4d0wm45k/glass-detect",
        "colonelpanichacks/flock-you",
        "yjeanrenaud/yj_nearbyglasses",
        "NullPxl/banrays",
        "colonelpanichacks/ouispy-detector",
        "LuxStatera/flock-hunter-cyd-wifi",
        "haxorthematrix/BLEPTD",
        "RamboRogers/esp32-bluetooth-scanner"
    };
    int y = 62;
    ...
    tft.setTextColor(TFT_CYAN, TFT_BLACK);
    tft.setCursor(8, 153);
    tft.print("DATABASE CREDITS:");
    tft.setTextColor(TFT_WHITE, TFT_BLACK);
    tft.setCursor(8, 165);
    tft.print("Bluetooth SIG Assigned Numbers");
    tft.setCursor(8, 176);
    tft.print("Nordic Semiconductor Bluetooth DB");

    tft.setTextColor(TFT_RED, TFT_BLACK);
    tft.setCursor(8, 190);
    tft.print("DESIGNED TO COMPLY WITH THE");
    tft.setCursor(8, 201);
    tft.print("NSW SURVEILLANCE DEVICES ACT 2007");
...
void drawBootLegalNotice() {
    // Credits are shown on the preceding build/credits page. Keep this page
    // focused on the existing legal/privacy notice and require a second BOOT press.
    drawLegalPrivacyNotice(false, "TO BEGIN SCANNING:", "PRESS BOOT");
}
...
void drawSentryLegalNotice() {
    ...
    drawLegalPrivacyNotice(false, "TO VIEW SENTRY STATUS:", "PRESS BOOT");
}

void waitForBootButtonToStart() {
    // GPIO0 BOOT is INPUT_PULLUP. Ignore a button already held during power-up
    // and require a fresh, debounced press for each startup page.
    while (digitalRead(PAGE_BUTTON_PIN) == LOW) delay(10);
    delay(80);

    for (;;) {
        if (digitalRead(PAGE_BUTTON_PIN) == LOW) {
            delay(40);
            if (digitalRead(PAGE_BUTTON_PIN) == LOW) break;
        }
        delay(10);
    }

    // Require release so one long press cannot skip both startup pages.
    while (digitalRead(PAGE_BUTTON_PIN) == LOW) delay(10);
    delay(80);
}
...
void handleSentryBootGate() {
    if (!sentryModeActive || sentryEntering) return;

    if (!sentryDashboardAwake) {
        // First BOOT press from the fully dark Sentry state shows the legal notice.
        drawSentryLegalNotice();
        return;
    }

    if (sentryLegalNoticeVisible) {
        // Second deliberate BOOT press advances to the normal Sentry dashboard.
        wakeSentryDashboard();
        return;
    }

    // Once the dashboard is visible, BOOT returns to its existing Sentry
    // double-press exit role; single presses are handled by updatePageButton().
}
...
void updateSentryMode() {
    ...
    // V8: Sentry legal/dashboard wake gates use the physical BOOT button.
    // Touch is deliberately not used for these gates.
    if (sentryDashboardAwake) {
...
void showScanningNow() {
    ...
    tft.setTextSize(2);
    tft.setCursor(8, 20);
    tft.print("ESP GLASSES SCANNER V8");
    ...
    // External BLE database status. The exact failure wording is intentionally
    // explicit because the CYD is then operating from hard-coded fallbacks only.
    BleDbClientState dbState = bleDbClientState();
    if (dbState == BLE_DB_CLIENT_ONLINE) {
        tft.setTextColor(TFT_GREEN, TFT_BLACK);
        tft.setCursor(6, 174);
        tft.print("DATABASE: ONLINE - LOCAL FALLBACKS ACTIVE");

        if (bleDbBusyModeIsActive()) {
            tft.setTextColor(TFT_RED, TFT_BLACK);
            tft.setCursor(6, 188);
            tft.print("BUSY MODE - NEAR DEVICES FIRST");
        }
    } else if (dbState == BLE_DB_CLIENT_STARTING) {
        tft.setTextColor(TFT_YELLOW, TFT_BLACK);
        tft.setCursor(6, 174);
        tft.print("DATABASE: STARTING - LOCAL FALLBACKS ACTIVE");
    } else {
        tft.setTextColor(TFT_RED, TFT_BLACK);
        tft.setCursor(6, 174);
        tft.print("DATABASE FAIL:");
        tft.setCursor(6, 188);
        tft.print("RESULT INACCURATE");
    }
...
void showStatusPage() {
    ...
    BleDbClientState dbStatus = bleDbClientState();
    tft.setTextColor(dbStatus == BLE_DB_CLIENT_ONLINE ? TFT_GREEN :
                     (dbStatus == BLE_DB_CLIENT_STARTING ? TFT_YELLOW : TFT_RED), TFT_BLACK);
    tft.setCursor(190, 59);
    tft.print("DATABASE: ");
    tft.print(bleDbClientStateText());
...
void updatePageButton() {
    ...
    if (pressed != lastPageButtonPressed &&
        ...
        if (pressed) {
            ...
            if (currentPage == PAGE_STATUS) {
                // V8 Sentry BOOT gates take precedence while the display is dark
                // or the legal notice is visible. These presses cannot also arm
                // the double-BOOT exit gesture.
                if (sentryModeActive && heldMs < PAGE_BUTTON_SHORT_MAX_MS &&
                    (!sentryDashboardAwake || sentryLegalNoticeVisible)) {
                    statusShortClickPending = false;
                    statusSecondClickArmed = false;
                    handleSentryBootGate();
                    return;
                }
                ...
                // Once the Sentry dashboard is visible, retain the existing
                // 2x BOOT exit gesture. A single short press only refreshes the
                // dashboard timeout after the double-press window expires.
                if (sentryModeActive) {
    ...
    if (statusShortClickPending && !pressed &&
        ...
        if (sentryModeActive && sentryDashboardAwake && !sentryLegalNoticeVisible) {
            wakeSentryDashboard();
            return;
        }
    }
...
class GlassholeScanCallbacks : public BLEAdvertisedDeviceCallbacks {
    void onResult(BLEAdvertisedDevice advertisedDevice) override {
        ...
        // External BLE database lookup is strictly asynchronous. The raw observed
        // MAC may exist in this RAM queue / UART request for exact matching, but it
        // is never written to SD, LittleFS, NVS/Preferences, or Serial output.
        // Queue saturation drops the external lookup rather than delaying BLE.
        uint32_t dbLocalDeviceHash = sessionDeviceHashForAddress(logSnapshot.address);
        queueBleDatabaseObservation(
            dbLocalDeviceHash,
            logSnapshot.address,
            logSnapshot.addressType,
            logSnapshot.companyId,
            logSnapshot.rssi,
            logSnapshot.name,
            logSnapshot.manufacturerHex,
            logSnapshot.serviceUUIDs
        );
...
void printSerialSystemStats(uint32_t nowMs) {
    ...
    Serial.println();
    Serial.println("============================================================");
    Serial.println("ESP GLASSES SCANNER - SYS STAT");
    Serial.println("============================================================");
    ...
    Serial.print("BLE database: ");
    Serial.println(bleDbClientStateText());
    Serial.print("DB queries queued/sent/results/hits/drops/timeouts: ");
    Serial.printf("%lu/%lu/%lu/%lu/%lu/%lu\n",
                  (unsigned long)bleDbQueriesQueued,
                  (unsigned long)bleDbQueriesSent,
                  (unsigned long)bleDbResultsReceived,
                  (unsigned long)bleDbMatchedResults,
                  (unsigned long)bleDbQueueDrops,
                  (unsigned long)bleDbTimeouts);
...
void setup() {
    ...
    // Two deliberate BOOT-button gates. There are no automatic timers:
    //   1) build / credits / NSW legal-design notice
    //   2) existing legal / privacy notice
    // BLE scanning is not initialized or started until both fresh BOOT presses occur.
    drawBootCreditsPage();
    waitForBootButtonToStart();
    drawBootLegalNotice();
    waitForBootButtonToStart();
    ...
    // Start the optional BLE database coprocessor link. Failure does not block
    // scanner startup: every existing hard-coded CYD rule remains authoritative.
    bool databaseClientStarted = initBleDatabaseClient();
    Serial.printf("BLE database coprocessor client: %s\n",
                  databaseClientStarted ? "STARTING" : "INIT FAIL - LOCAL RULES ONLY");
    ...
    Serial.println("BUSY DB priority: >20 unique/min enters, <15/min exits; 5-sample median RSSI, near first.");
    Serial.println();
    ...
    // v8 retains the validated v6.3 RF privacy path: create and confirm a fresh PRIVATE NRPA before the
...
void loop() {
    ...
    // Drain external database results outside the BLE callback. Database state
    // changes may refresh the current page but can never stop local scanning.
    processBleDatabaseResults();
    if (bleDbConsumeStatusChanged() &&
        !warningScreenActive && !micCamAlert.active && !suspiciousAlert.active &&
        !manualLogNoticeActive && !sentryModeActive) {
        showSelectedPage();
    }

    // v8 retains the validated v6.3 behavior: rotate/recover the CYD's own PRIVATE NRPA only while BLE is idle.
