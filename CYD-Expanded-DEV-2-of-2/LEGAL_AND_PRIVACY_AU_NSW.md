...
## Build-specific scope — CYD Expanded DEV 2 of 2 (database ESP32)

**2 OF 2 MUST BE FLASHED TO THE SEPARATE CLASSIC ESP32 DATABASE COPROCESSOR.**

This half is not an independent BLE scanner in the current architecture. It performs local UART database/reference lookups for the CYD.

Current design:
- no BLE scanning on the database ESP;
- no Wi-Fi scanning on the database ESP;
- no observed-device SD/LittleFS/SPIFFS/NVS log;
- raw observed BLE MAC addresses may exist transiently in RAM/UART for matching;
- raw observed BLE MAC addresses must not be intentionally persisted or printed to USB serial.

### Database/reference legal scope

Bluetooth Company Identifiers, service references and curated device rules are **reference evidence**, not legal proof of physical product identity.

A Company Identifier identifies an assigned organisation. It does not, by itself, prove that an observed physical device is smart glasses, a camera, microphone or recording device.

Generated/reference datasets may have their own copyright, licence and attribution requirements. Preserve required third-party notices and verify the licence of any dataset distributed with a public release.

### No expansion of legal authority

Adding the database coprocessor increases reference capacity; it does not increase the operator's legal authority to monitor, retain, identify or act on observations.

### Experimental status

This coprocessor is working/development firmware and remains mostly untested as part of the Expanded system. Do not rely on it as the sole basis for security, safety, enforcement, disciplinary or compliance decisions.
