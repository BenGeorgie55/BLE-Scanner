...
## Build-specific scope — ESP32-WROOM v1.1 NeoPixel

This is the **NeoPixel-only** classic ESP32-WROOM branch. It does not use an OLED.

Current-source behaviour relevant to privacy/legal use:
- LittleFS candidate/session logging is present;
- the `LOG` serial command is present;
...
- privacy-address failure and public fallback are visibly indicated through purple LED states.

Because retained logs can survive reboot/power loss, they should be handled as potentially sensitive operational records.

Recommended public positioning: low-cost portable consumer/privacy-awareness detector. Organisational use should follow the workplace, school, health, aged-care and secure-facility cautions above.
